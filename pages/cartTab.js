...
class keranjang extends Component {
  ...
  addQty = (data)=> {
    let index = this.state.dataDummyCart.findIndex(i => i.id == data.id)
    let cartAwal = this.state.dataDummyCart
  ...
  removeFromcart = (datax)=> {
    let checkData = this.state.dataDummyCart
    let endData =  checkData.filter(function(data) {
      return data.id != datax.id;
    });
    this.props.dispatch({ type: "ADD_CART", payload: endData });
    this.reloadScreen()
  }
  render() {
